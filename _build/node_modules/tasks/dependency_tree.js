...
const analyzer = require("./low_level/source_code_analyzer");
...
const { minimatch: matchesGlob } = require("minimatch");
...
module.exports = class DependencyTree {
	...
	async dependenciesForAsync(filename, notifyFn = () => {}) {
		...
		try {
			...
			if (err.code === "ENOENT") errors.push(`Attempted to find dependencies for file that doesn't exist: ${filename}`);
			else throw err;
		}
...
async function populateDependenciesAsync(self, notifyFn, filename, dependencySet, errors, notFoundError) {
	...
	await Promise.all(dependencies.map(async ({ dependency, error }) => {
		...
		const dependencyNotFound = notFoundMessage(filename, dependency.path, dependency.line, dependency.source);
...
function notFoundMessage(parent, child, line, source) {
	return `Cannot find dependency '${child}'\n` +
		`Referenced on line ${line} of '${parent}':\n` +
		`  ${source.trim()}`;
}
