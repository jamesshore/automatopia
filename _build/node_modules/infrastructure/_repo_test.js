...
module.exports = suite(({ describe }) => {
	...
	describe("Heroku commands", ({ it }) => {

		it("checks if user is logged in", async () => {
			const { repo, shell } = createRepo([{
				code: 1,    // cause an error the first time we call heroku
			}, {
				code: 0,    // no error the second time
			}]);

			assert.equal(await repo.isHerokuLoggedIn(), false, "not logged in");
			assertCommand(shell, "heroku", "auth:whoami");

			assert.equal(await repo.isHerokuLoggedIn(), true, "logged in");
		});

		it("logs in to Heroku", async () => {
			const { repo, shell } = createRepo();

			await repo.herokuLogin();
			assertCommand(shell, "heroku", "auth:login");
		});

		it("deploys to Heroku", async () => {
			const { repo, shell } = createRepo();
			await repo.herokuDeployAsync("remote-repo", "my-branch");
			assertCommand(shell, "git", "push", "remote-repo", "my-branch:master");
		});

		it("rolls back Heroku deploy", async () => {
			const { repo, shell } = createRepo();
			await repo.herokuRollbackAsync("my-app");
			assertCommand(shell, "heroku", "rollback", "--app", "my-app");
		});

		it("imitates Heroku deployment state", async () => {
			const { repo, shell } = createRepo();
			await repo.resetToHerokuDeploymentStateAsync();

			assertCommands(shell, [
				// Heroku starts with just the repo files, so we erase everything that isn't in the repo
				[ "git", "clean", "-fdx" ],

				// Heroku runs `npm install` and that can change installed files, so we do too
				[ "npm", "install", "--no-audit" ],

				// Heroku prunes devDependencies, so we do too
