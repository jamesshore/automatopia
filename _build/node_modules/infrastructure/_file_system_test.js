...
export default test(({ beforeAll, beforeEach, describe }) => {
	...
	describe("Reading and writing", ({ it, describe }) => {
		...
		it("reads and writes files", async () => {
			...
			assert.equal(readContents, "my file contents");
		});
	...
	//
	// 		assert.equal(await watchPromise, {
	...
	describe("File trees", ({ it, describe }) => {
		...
		it("provides files in a directory, recursively", async () => {
			...
			assert.dotEquals(await fileSystem.readFileTreeAsync(TEST_DIR), FileTree.create([
				`${TEST_DIR}/file1`,
				`${TEST_DIR}/file2`,
				`${TEST_DIR}/sub2/file4`,
				`${TEST_DIR}/sub1/file3`,
		...
		it("has variant that doesn't fail if directory doesn't exist", async () => {
			...
			assert.equal(await fileSystem.readPossibleFileTreeAsync(TEST_DIR), FileTree.create([]));
		});
		...
		describe("Nulled", ({ it }) => {
			...
			it("provides appropriate subset of configured files", async () => {
				...
				assert.dotEquals(await fileSystem.readFileTreeAsync("/my_parent"), FileTree.create([
					"/my_parent/file1",
					"/my_parent/file2",
					"/my_parent/my_child/file3",
					"/my_parent/my_child/file4",
			...
			it("doesn't match partial directories", async () => {
				...
				assert.dotEquals(await fileSystem.readFileTreeAsync("/legit"), FileTree.create([
					"/legit/file1",
				]));
			});
			...
			it("works if path ends in /", async () => {
				...
				assert.dotEquals(await fileSystem.readFileTreeAsync("/sub/"), FileTree.create([
					"/sub/file",
				]));
			});
	...
	describe("Modification times", ({ it }) => {
		...
		describe("Nulled", ({ it }) => {
			...
			it("allows nulled file modification time to be configured", async () => {
				...
				assert.equal(await fileSystem.getFileModificationTimeAsync("/default"), DEFAULT, "default");
				assert.equal(await fileSystem.getFileModificationTimeAsync("/configured"), new Date(1500), "configured");
			});
	...
	describe("Directory comparisons", ({ it }) => {
		...
		it("says which files have been added", async () => {
			...
			assert.equal(await fileSystem.compareDirectoriesAsync("/src", "/dest"), {
				added: [
		...
		it("takes a function that rewrites input filenames to output filenames", async () => {
			...
			assert.equal(await fileSystem.compareDirectoriesAsync("/src", "/dest", rewrite), {
				added: [
		...
		it("works even if directories don't exist", async () => {
			...
			await assert.equal(await fileSystem.compareDirectoriesAsync("/not_exists", "/exists"), {
				added: [],
				removed: [
			...
			await assert.equal(await fileSystem.compareDirectoriesAsync("/exists", "/not_exists"), {
				added: [
